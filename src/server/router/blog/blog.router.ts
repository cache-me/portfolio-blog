...
export const blogRouter = {
  ...
  addMedia: adminOnly
    .input(addBlogMediaInput)
    .handler(async ({ input, context }) => {
      const service = new BlogService(context.db);
      return service.addMedia(input);
    }),

  reorderMedia: adminOnly
    .input(reorderBlogMediaInput)
    .handler(async ({ input, context }) => {
      const service = new BlogService(context.db);
      return service.reorderMedia(input);
    }),

  deleteMedia: adminOnly
    .input(deleteBlogMediaInput)
    .handler(async ({ input, context }) => {
      const service = new BlogService(context.db);
      return service.deleteMedia(input.id);
    }),
